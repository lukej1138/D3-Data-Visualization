...
function _10(network,d3,width,height,opacity,size,color,forceBoundary,drag,invalidation)
{
...
function _network(selectedPapers,getKeyByValue,getKey,findOrAdd,netClustering)
{
  ...
  //Find authors that published 2023
  const recentAuthors = selectedPapers
    .filter(paper => paper.Year === 2023)
    .map(paper => paper.AuthorNames.split(";"))
    .flat();
  
  // Find papers from the past 20 years
  const recentPapers = selectedPapers.filter(paper => paper.Year <= 2023 && paper.Year >= 2003);
  
  // Count occurrences of each author in the last 20 years
  ...
  const paperToSort = recentPapers.filter(paper=> paper.AuthorNames.toLowerCase().includes(authorToSortFor.toLowerCase()))
...
haven't published in 2023 (except for the authorToSortFor).
  ...
  */
  ...
  //Filter papers from past 20 years that only include this most-relevant author
...
export default function define(runtime, observer) {
  ...
  main.variable(observer()).define(["network","d3","width","height","opacity","size","color","forceBoundary","drag","invalidation"], _10);
